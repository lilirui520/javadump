...
func main() {
	...
	if exist {
		var url = uploadStorage(dumpFile, pod)
...
// Storage
func uploadStorage(file string, pod string) string {
...
// 发送IM 工具
func notifyFunc(fileurl string, pod string) {
	if enabledd {
		//notify.SendDingtalk(fileurl)
		notify.SendDingTalkWithSign(fileurl, pod)
	}
	if enablewx {
